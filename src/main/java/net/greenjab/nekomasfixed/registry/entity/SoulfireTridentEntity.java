...
public class SoulfireTridentEntity extends PersistentProjectileEntity {
	private static final TrackedData<Byte> LOYALTY = DataTracker.registerData(SoulfireTridentEntity.class, TrackedDataHandlerRegistry.BYTE);
	private static final TrackedData<Boolean> ENCHANTED = DataTracker.registerData(SoulfireTridentEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
	private boolean dealtDamage = false;
	public int returnTimer;

	public SoulfireTridentEntity(EntityType<? extends SoulfireTridentEntity> entityType, World world) {
		super(entityType, world);
	}

	public SoulfireTridentEntity(World world, LivingEntity owner, ItemStack stack) {
		super(EntityTypeRegistry.SOULFIRE_TRIDENT, owner, world, stack, null);
		this.dataTracker.set(LOYALTY, this.getLoyalty(stack));
		this.dataTracker.set(ENCHANTED, stack.hasGlint());
	}

	public SoulfireTridentEntity(World world, double x, double y, double z, ItemStack stack) {
		super(EntityTypeRegistry.SOULFIRE_TRIDENT, x, y, z, world, stack, stack);
		this.dataTracker.set(LOYALTY, this.getLoyalty(stack));
		this.dataTracker.set(ENCHANTED, stack.hasGlint());
	}
	...
	protected void onEntityHit(EntityHitResult entityHitResult) {
		...
		if (entity.sidedDamage(damageSource, f)) {
			...
			if (this.getEntityWorld() instanceof ServerWorld serverWorld) {
				EnchantmentHelper.onTargetDamaged(serverWorld, entity, damageSource, this.getWeaponStack(), /* method_63013 */ item -> this.kill(serverWorld));
			}
	...
	protected ItemStack getDefaultItemStack() {
		return new ItemStack(ItemRegistry.SOULFIRE_TRIDENT);
	}
